...
class AgentRealistic:
    def __init__(self, agent_host, agent_port, mission_type, mission_seed, solution_report, state_space):
        ...
        self.goal_location = state_space.state_locations[state_space.goal_id]
...
class AgentSimple:
    ...
    def run_agent(self):
        ...
        # -- test the start_location and the goal_location
        print "start_location: %s" % {self.state_space.state_locations[self.state_space.start_id]}
        print "goal_localtion: %s" % [self.state_space.state_locations[self.state_space.goal_id]]
        # print "locations: %s" %(self.state_space.state_locations)

        # -- the set the agent_simple map use to initial the search area
        ...
        state_map = {}
        for state in self.state_space.state_locations:
            state_map[state] = dict(self.state_space.state_actions[state])

        agent_simple_map = UndirectedGraph(state_map)
        agent_simple_map.locations = {}
        for state in self.state_space.state_locations:
            agent_simple_map.locations[state] = self.state_space.state_locations[state]

        # -- initial the agent_simple problem based on AIMA toolbox search
        agent_simple_problem = GraphProblem(self.state_space.start_id, self.state_space.goal_id, agent_simple_map)
        node = astar_search(problem=agent_simple_problem, h = None)
        ...
        while state_t.is_mission_running:
            ...
            try:
                ...
                if target_node.state == self.state_space.goal_id:
                    ...
                    x_new = self.state_space.state_locations[self.state_space.goal_id][0]
                    z_new = self.state_space.state_locations[self.state_space.goal_id][1] - 0.5
                else:
            ...
            print "State: observations, rewards received:", state_t.number_of_observations_since_last_state, state_t.number_of_rewards_since_last_state
            print "\t x,y,z,yaw,pitch:", xpos, ypos, zpos, yaw, pitch

        # --------------------------------------------------------------------------------------------
        print
        print "Mission has ended ... either because time has passed or goal reached (i.e. cumulative reward = 1000)"
...
class AgentHelper:
    ...
    def run_agent(self):
        ...
        if state_t.is_mission_running:
            ...
            # to all states and detect whether you  receieve a
